...
@endpush
@section('content')
    <h1>Edit Sidebar Menu</h1>

    <form action="{{ route('sidebar-menu.update', $sidebarMenu) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label>Title <span class="text-danger">*</span></label>
            <input type="text" name="title" class="form-control" required value="{{ old('title', $sidebarMenu->title) }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="group" class="form-control" value="{{ old('group', $sidebarMenu->group) }}">
            <small class="text-muted">Contoh: Menu, Pages, Administrator</small>
        </div>

        <div class="mb-3">
            <label>Order <span class="text-danger">*</span></label>
            <input type="number" name="order" class="form-control" required value="{{ old('order', $sidebarMenu->order) }}">
        </div>

        <div class="mb-3 form-check">
            <input type="checkbox" name="is_submenu" class="form-check-input" id="is_submenu" value="1" {{ old('is_submenu', $sidebarMenu->is_submenu) ? 'checked' : '' }}>
        ...
        <div class="mb-3">
            ...
            <select name="parent_id" class="form-control">
                ...
                @foreach($parents as $parent)
                    <option value="{{ $parent->id }}" {{ old('parent_id', $sidebarMenu->parent_id) == $parent->id ? 'selected' : '' }}>
                        {{ $parent->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="roles" class="form-control" value="{{ old('roles', $sidebarMenu->roles) }}">
